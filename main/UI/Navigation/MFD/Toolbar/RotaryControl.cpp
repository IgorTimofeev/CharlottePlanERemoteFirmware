...
namespace pizda {
	...
	void RotaryControl::onTick() {
		...
		if (variantSelectMode) {
			RC::getInstance().playFeedback(&Sounds::engaged);
		}
		else {
			RC::getInstance().playFeedback(&Sounds::disengaged);
		}
	}
...
//
//		renderer->putPixel(Point(x + 1, y - 1), &Theme::fg1);
//		renderer->putPixel(Point(x, y), &Theme::fg1);
//		renderer->putPixel(Point(x + 1, y + 1), &Theme::fg1);
//
